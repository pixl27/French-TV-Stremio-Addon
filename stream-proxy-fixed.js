...
class StreamProxy {
    ...
    setupRoutes() {
        ...
        this.app.get('/stream/:channelId/playlist.m3u8', async (req, res) => {
            try {
                ...
                // Refresh auth if it's older than 30 seconds or doesn't exist
                if (!authInfo || (now - authInfo.timestamp) > 30000) {
    ...
    async handleSegmentRequest(channelId, segmentName, req, res) {
        try {
            ...
            // Refresh auth if it's older than 8 seconds or doesn't exist
            if (!authInfo || (now - authInfo.timestamp) > 8000) {
